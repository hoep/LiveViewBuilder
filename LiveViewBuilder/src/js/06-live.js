  ...
  function _collectIds(w,add){ // alle Variablen-IDs eines Widgets an add() geben
    add(w.varId);add(w.varId2);add(w.varId3);add(w.cvActId);add(w.cvAzB);add(w.cvAzE);add(w.cvElv);add(w.cvBlockVid);add(w.cmpVid);add(w.dVid);add(w.ackVid);add(w.visVar);add(w.condVar);add(w.vTemp);add(w.vCond);add(w.vHum);add(w.vWind);add(w.vRain);add(w.ssAz);add(w.ssEl);add(w.ssRad);add(w.ssRainV);add(w.ssSnowV);add(w.ssPtypeV);add(w.ssFogV);add(w.ssFogStateV);add(w.ssWindV);add(w.ssRainSensV);add(w.ssTempV);add(w.ssDewV);add(w.ssHumV);add(w.ssWetV);add(w.ssCloudV);add(w.ssWxJson);add(w.vStorm);add(w.vStormDist);add(w.vStormAge);add(w.vStormRate);add(w.vRainRate);add(w.vRainDay);add(w.vFog);add(w.vFogFsi);add(w.ssStormV);add(w.ssStormDistV);add(w.wxFog);add(w.wxFogState);add(w.wxPrecip);add(w.wxRainRate);add(w.wxStorm);add(w.wxStormDist);add(w.wxSnow);
