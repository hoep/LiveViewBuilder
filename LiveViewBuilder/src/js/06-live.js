  ...
  var _layoutV=0; // zuletzt gesehene Layout-Version (mtime) — Run-Ansicht lädt bei Änderung neu
  function pollVals(){
    if(!_vidx)buildVidx();
    ...
    fetch('?api=val&ids='+ids.join(',')+'&since='+_pvSince,{cache:'no-store'}).then(function(r){return r.json();}).then(function(j){
      if(!j)return;if(j.ts)_pvSince=j.ts;
      if(j.lv){if(_layoutV&&j.lv!==_layoutV&&RUN){location.reload();return;}_layoutV=j.lv;} // Run-Ansicht bei gespeicherter Layout-Aenderung hart neu laden
      if(!j.values)return;
