  ...
  function widgetDataId(w,id){
    if(w.varId===id||w.varId2===id||w.varId3===id||w.cvActId===id||w.cvAzB===id||w.cvAzE===id||w.cvElv===id||w.cmpVid===id||w.ackVid===id||w.condVar===id||w.vTemp===id||w.vCond===id||w.vHum===id||w.vWind===id||w.vRain===id||w.ssAz===id||w.ssEl===id||w.ssRad===id||w.ssRainV===id||w.ssSnowV===id||w.ssPtypeV===id||w.ssFogV===id||w.ssWindV===id||w.ssRainSensV===id)return true;
  ...
  // ---- String-Verkettung: "=#<ID>."°C ".#<ID>."%"" -> Text aus Variablen + Literalen (PHP-Punkt-Operator).
  ...
  function _collectIds(w,add){ // alle Variablen-IDs eines Widgets an add() geben
    add(w.varId);add(w.varId2);add(w.varId3);add(w.cvActId);add(w.cvAzB);add(w.cvAzE);add(w.cvElv);add(w.cmpVid);add(w.ackVid);add(w.visVar);add(w.condVar);add(w.vTemp);add(w.vCond);add(w.vHum);add(w.vWind);add(w.vRain);add(w.ssAz);add(w.ssEl);add(w.ssRad);add(w.ssRainV);add(w.ssSnowV);add(w.ssPtypeV);add(w.ssFogV);add(w.ssWindV);add(w.ssRainSensV);
