  ...
  var DOKU_GROUPS = [
    ...
    ['Schalten & Bedienen',     ['switch','light','button','tile','checkbox','select','slider','colorpick','textbox','cover','shading','shadeprofile','thermostat','heatplan','weekedit','alarm','bot','timer','eventctl']],
    ...
    (function(){
      ...
      add({type:'text', x:DOKU_PAD, y:y, w:W, h:132, bgT:true, fsz:13, fg:'#39d08a',
           label:'=45552+49633              Summe zweier Zähler (z. B. PV1 + PV2)\n'
                +'=(#<ID>+#<ID>)/2        Mittel zweier Leistungen (Klammern erlaubt)\n'
                +'=45552-49633              Differenz zweier Zähler\n'
                +'=53289*0.06               Variable mal Konstante (z. B. Preis)\n'
                +'=(#<ID>+#<ID>)*100/#<ID>   gemischt mit Klammern und Konstante'}); y += 144;
      ...
      add({type:'text', x:DOKU_PAD, y:y, w:W, h:132, bgT:true, fsz:13, fg:'#7d9099',
           label:'• Operatoren + − * / und Klammern ( ).\n'
                +'• Eine Zahl ist eine VARIABLE, wenn sie mit # beginnt (#<ID>) oder fünfstellig ist (≥ 10000). '
      ...
      add({type:'text', x:DOKU_PAD, y:y, w:W, h:96, bgT:true, fsz:13, fg:'#39d08a',
           label:'=#<ID>."°C ".#<ID>."%"       Temperatur und Luftfeuchte in einer Zeile\n'
                +'=#<ID>." / ".#<ID>." W"      zwei Leistungen mit Trenner\n'
                +'="Zähler: ".#<ID>." kWh"      fester Vortext plus Variable'}); y += 108;
      add({type:'text', x:DOKU_PAD, y:y, w:W, h:24, bgT:true, fsz:15, label:'Regeln'}); y += 30;
