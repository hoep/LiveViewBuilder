  ...
  function switchView(name){if(!store.views[name])return;store.current=name;state=store.views[name];if(!state.page)state.page={w:1440,h:900};if(!state.widgets)state.widgets=[];selId=null;sel={};reseq();refreshViewSel();setCanvas();invalidateSC();_scMode='';document.body.classList.remove('reflow');restoring=true;render();restoring=false;renderProps();resetHist();_pvSince=0;pollVals();
    state.widgets.forEach(function(w){try{if(w.type==='camera'||w.type==='campro'){w._lastCam=0;refreshCam(w);}else if(w.type==='html'){if(w.htmlSrc==='custom')setHtmlContent(w,w.html||'');else fetchHtml(w);}else if(w.type==='msglog'&&typeof fetchMsgs==='function')fetchMsgs(w);}catch(e){}});} // Kamera/HTML/Meldungen nach Seitenwechsel sicher neu laden
