  ...
  function sfTracks(ax,D){
    ...
    var minT=Math.max(6,0.008*D); // kleinere Gaps zwischen eng benachbarten Widgets erhalten (sonst verschmelzen sie)
  ...
  function sfDistribute(T,view,s0,c){
    // Uniform mit s0 skalieren -> Widget-Seitenverhältnisse bleiben exakt erhalten.
    var base=0;T.forEach(function(t){t.px=t.size*s0;base+=t.px;});
    var spare=view-base;
    var gapPx=0;T.forEach(function(t){if(t.w<=0)gapPx+=t.px;}); // Summe der Zwischenraum-Spuren (ohne Widget)
    if(spare>0.5&&gapPx>0.5){
      // freien Platz proportional in die Zwischenräume geben -> Widgets rücken auseinander, keine Verzerrung
      T.forEach(function(t){if(t.w<=0)t.px+=spare*(t.px/gapPx);});
      var cur=0;T.forEach(function(t){t.pos=cur;cur+=t.px;});
    }else{
      // keine Zwischenräume (oder kein Platz) -> gleichmäßig zentrieren (Letterbox-Rand)
      var cur2=Math.max(0,spare)/2;T.forEach(function(t){t.pos=cur2;cur2+=t.px;});
    }
  }
