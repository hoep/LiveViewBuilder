    ...
    loadAssoc(w.varId,function(d){var cur=$('#assocBox');if(cur!==box)return;
      ...
      var rows=d.assocs.map(function(a){var key=String(a.v),rr=assocResolved(w,a),col=rr.color;
        return '<div class="arow"><span class="aic" style="color:'+(col||'var(--accent)')+'">'+(rr.icon?iconSVG(rr.icon):'')+'</span>'
          +'<input type="color" class="aswc" data-akey="'+esc(key)+'" value="'+esc(col||'#888888')+'" title="Farbe für Icon + Text">'
          +'<span class="anm">'+esc(a.name||key)+'</span><span class="aval">'+esc(key)+'</span>'
          +'<button class="btn aibtn" data-akey="'+esc(key)+'" style="padding:3px 7px;font-size:11px">Icon</button>'
          +'<button class="btn acclr" data-akey="'+esc(key)+'" style="padding:3px 6px;font-size:11px" title="Farbe zurücksetzen">×</button></div>';}).join('');
      ...
      $$('.aswc',box).forEach(function(inp){inp.oninput=function(){var k=inp.getAttribute('data-akey');if(!w.assocMap)w.assocMap={};if(!w.assocMap[k])w.assocMap[k]={};w.assocMap[k].color=inp.value;w.assocOn=true;var ar=inp.closest('.arow');if(ar){var ai=ar.querySelector('.aic');if(ai)ai.style.color=inp.value;}refreshAssocLive(w);commit();};}); // Farbe (Icon+Text) überschreiben
      $$('.acclr',box).forEach(function(b){b.onclick=function(){var k=b.getAttribute('data-akey');if(w.assocMap&&w.assocMap[k])delete w.assocMap[k].color;render();renderProps();refreshAssocLive(w);commit();};}); // Farb-Override entfernen
    });}
