  ...
  defWidget('value',{
    ...
    live:function(w,el,id,d,base,txt,on){var v=$('[data-role=val]',el);if(!v)return;v.textContent=txt;
      ...
      var _match=null;if(w.vassoc&&w.vassoc.length){_match=w.vassoc.filter(function(a){return String(a.v)===String(d.v);})[0];if(_match){if(_match.text!=null&&_match.text!=='')v.textContent=(w.pre||'')+_match.text+(w.suf||'');if(_match.color){var _mc=_skinColor(_match.color)||_match.color;v.style.color=_mc;}}}
      if(w.vaFill){ // ganze Kachel in der Zustandsfarbe tönen
        if(_match&&_match.color){var _c=_skinColor(_match.color)||_match.color;el.style.background='color-mix(in oklab,'+_c+' 16%,var(--surface))';el.style.borderColor='color-mix(in oklab,'+_c+' 45%,var(--line))';v.style.color='color-mix(in oklab,'+_c+' 85%,var(--text))';}
        else{el.style.background=w.bg||'';el.style.borderColor='';}
      }}
  });
