  ...
  function spanFetch(w,el){var st=spanSt(w);
    if(typeof DOKU!=='undefined'&&DOKU){spanApply(st,spanDemoAll());st.sun={az:214,elev:38,ts:0};st.geo={lat:48.2082,lon:16.3738};st.loaded=true;st.err='';spanRepaint(w,el);return;}
  ...
  defWidget('shadingpanel',{
    ...
    props:function(w){
      ...
      return '<div style="font-size:11px;color:var(--muted);padding:2px 2px 6px">Sonnenstand aus Location #<ID> (verifiziert). Titel oben = Label-Feld; Raumnamen/Temp-Variablen im Backend #<ID>.</div>'
