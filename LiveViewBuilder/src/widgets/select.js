  ...
  function _selMode(w){return w.smode==='list'?'list':'btn';}
  ...
  function _selBody(w){
    ...
    return '<div class="hsel">'+opts.map(function(o){
      return '<button class="hselb" data-selval="'+esc(String(o.value!=null?o.value:''))+'"'
        +(o.color?' style="--sc:'+esc(o.color)+'"':'')+'>'+esc(o.text||String(o.value))+'</button>';
    }).join('')+'</div>';
  }
    ...
    fetch('?api=assoc&id='+w.varId,{cache:'no-store'})
      ...
      .then(function(j){
        _selAssoc[w.varId]=(j&&j.assocs||[]).map(function(a){
          return {value:a.v,text:a.name,color:a.color||''};});
  ...
  defWidget('select',{
    ...
    props:function(w){
      return row('Darstellung','<select id="pSelMode"><option value="btn"'+(_selMode(w)==='btn'?' selected':'')+'>Knopfreihe</option><option value="list"'+(_selMode(w)==='list'?' selected':'')+'>Klappliste</option></select>')
        ...
        +(w.optSrc==='profile'
          ? '<div style="font-size:11px;color:var(--muted);margin:2px 2px 5px">Die Optionen kommen aus den Zuordnungen des Profils und folgen ihm automatisch.</div>'
          : listEditor(w,'options','Optionen: Wert · Text · Farbe',[{k:'value',ph:'Wert'},{k:'text',ph:'Text'},{k:'color',type:'skincolor'}]));
    },
    wire:function(w){
      if($('#pSelMode'))$('#pSelMode').onchange=function(){w.smode=(this.value==='list'?'list':undefined);render();commit();};
