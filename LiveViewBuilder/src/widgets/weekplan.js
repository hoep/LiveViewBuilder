  ...
  defWidget('weekplan',{
    ...
    render:function(w){var nc=(_skinColor(w.nowColor||'warn')||'var(--warn)'); // Farbe der „jetzt"-Markierung (Default: Warnung)
      return '<div class="hwp" style="--wp-now:'+nc+'"><div class="hwphd">'+esc(w.label||'Wochenplan')+'</div><div class="hwpgrid" data-role="wpgrid"><div class="hwpempty">lädt …</div></div>'+(w.showTimes?'<div data-role="wptimes" style="font-size:10px;color:var(--muted);padding:4px 8px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"></div>':'')+'</div>';},
    props:function(w){return row('Schaltzeiten (heute)','<input type="checkbox" id="pWpTimes"'+(w.showTimes?' checked':'')+'>')
      ...
      +listEditor(w,'colors','Zustand · Farbe',[{k:'name',ph:'Zustand (z. B. Pumpe läuft)'},{k:'color',ph:'#00cdab / accent'}]);},
