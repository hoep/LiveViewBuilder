  ...
  defWidget('msglog',{
    ...
    props:function(w){return '<div class="pgh">Quelle</div>'
      ...
      +(w.src==='homematic'?(
         row('CCU-IP','<input id="pMsgHmIP" value="'+esc(w.hmIP||'')+'" placeholder="z. B. 192.168.1.20">')
    ...
    click:function(w,el,e){
      ...
      if(sw){var cur=_msgSrc(w),nx=(cur==='homematic')?'symcon':'homematic';try{localStorage.setItem('lvmsgsrc_'+w.id,nx);}catch(_){}render();fetchMsgs(w);return true;}
