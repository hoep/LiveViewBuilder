  ...
  defWidget('infolist',{
    ...
    render:function(w){return '<div class="hinfos">'+(w.items||[]).map(function(r){var rt=r.pill?'<span class="hpill '+esc(r.state||'ok')+'"><span class="hpd"></span>'+esc(r.pill)+'</span>':'<span class="hiv"'+(r.vid?' data-vid="'+r.vid+'"':'')+'>'+esc(r.value||'')+'</span>';var _ic=r.color?(_cssColorOrEmpty(r.color)||''):'';return '<div class="hinfo"><span class="hibi"'+(_ic?' style="color:'+_ic+';background:color-mix(in oklab,'+_ic+' 14%,var(--surface-2))"':'')+'>'+iconSVG(r.icon||'sensor')+'</span><span class="hin">'+esc(r.label||'')+(r.sub?'<small>'+esc(r.sub)+'</small>':'')+'</span>'+rt+'</div>';}).join('')+'</div>';},
