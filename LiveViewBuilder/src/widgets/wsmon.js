  ...
  function drawWsmon(w,root){
    ...
    box.innerHTML=why+arr.map(function(e){
      var d=_hzD(e.t),p=function(n){return ('0'+n).slice(-2);},tm=p(d.getHours())+':'+p(d.getMinutes())+':'+p(d.getSeconds());
  ...
  defWidget('wsmon',{
    label:'Live-Monitor', cat:'Anzeige', paletteIcon:'wlist', size:[300,200],
    ...
    render:function(w){
      ...
      // Abstand vor dem Poll-Punkt in em statt fest 6px: er folgt damit der Schriftgroesse der
      // Legende (--wf-lbl) und bleibt auf halber wie doppelter Kachel im gleichen Verhaeltnis.
      // Fuer diese Stelle gibt es keine eigene CSS-Regel, es entsteht also keine zweite Quelle.
      return '<div class="hws">'+(w.label?('<div class="hwshd"><span>'+escL(w.label)+'</span><span class="hwslg"><span class="hwsd ws"></span>WS<span class="hwsd poll" style="margin-left:.5em"></span>Poll</span></div>'):'')
